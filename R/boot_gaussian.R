################################################################################
#' @title Parametric Bootstrap of time-to-event data following a gaussian distribution
#'
#' @description Function generating bootstrap data according to a gaussian distribution (specified by a model parameter \eqn{\theta}),
...
#' @param theta parameter of the gaussian distribution, theta=(mean,sd)
...
boot_gaussian <- function(t0,B=1000,theta,C,N){

  boot <- numeric()

  #data simulating function
  simul <- function(N, mean, sd, rateC)
  {
    # survival time assuming a gaussian distribution
    Tlat <- rnorm(N, mean = mean, sd = sd)
  ...
  for(l in 1:B){
    data_rb <- simul(N=N, mean=theta[1], sd=theta[2], rateC=C)
    mod1b <- survreg(Surv(time,status)~1,data=data_rb,dist="gaussian")
